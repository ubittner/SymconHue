...
/**
 * @project       SymconHue/GroupedLight
 * @file          module.php
...
class HueGroupedLight extends IPSModule
{
    //Helper
    use ColorHelper;

    //Constant
    private const RESOURCE_GROUPED_LIGHT = 'ResourceGroupedLight';
    private const RESOURCE_SCENE = 'ResourceScene';
    ...
    public function Create()
    {
        ...
        //Scene
        $id = @$this->GetIDForIdent('Scene');
        $this->RegisterVariableString('Scene', $this->Translate('Scene'), '', 60);
        ...
        $this->RequireParent('{A8577857-5AC9-A684-C4CC-671A921E8BFE}');
    }
     ...
     * @throws Exception
     */
    public function SetGroup(string $Color, string $OptionalParameters): bool
    {
        $resourceID = $this->ReadPropertyString('ResourceID');
        if ($resourceID == '') {
            return false;
        ...
        $this->SendDebug(__FUNCTION__, 'Value: ' . $value, 0);
        $result = $this->SendData('PUT', self::RESOURCE_GROUPED_LIGHT, $resourceID, $value);
     ...
     * @throws Exception
     */
    public function UpdateGroupValues(): bool
    {
        $resourceID = $this->ReadPropertyString('ResourceID');
        if ($resourceID == '') {
            return false;
        }
        $result = $this->SendData('GET', self::RESOURCE_GROUPED_LIGHT, $resourceID, '');
    ...
    {
        $profile = MODULE_PREFIX . '.' . $this->InstanceID . '.Scene';
        if (IPS_VariableProfileExists($profile)) {
            IPS_DeleteVariableProfile($profile);
        }
        $result = json_decode($this->SendData('GET', self::RESOURCE_SCENE, '', ''), true);
        $data = $result['data'];
        $scenes = [];
        ...
        if (array_key_exists($roomZoneID, $scenes)) {
            ...
            foreach ($associations as $association) {
                IPS_SetVariableProfileAssociation($profile, $association[0], $association[1], $association[2], $association[3]);
            }
            $id = $this->GetIDForIdent('Scene');
    ...
    /**
     * Switches the group of lights off or on.
     *
     * @param bool $State
     * false =  off,
     * true =   on
     ...
     * @throws Exception
     */
    private function SwitchGroup(bool $State): bool
    {
        $resourceID = $this->ReadPropertyString('ResourceID');
        if ($resourceID == '') {
            return false;
    ...
    /**
     * Sets the brightness for the group of the lights.
     *
    ...
    /**
     * Sets the color for the group of the lights.
     *
     * @param int $Color
     *
     * @return bool
    ...
    /**
     * Sets the color temperature for the group of the lights.
     *
    ...
    private function SetColorTemperature(int $Temperature): bool
    {
        ...
        $duration = $this->GetValue('Transition') ? $this->GetValue('Transition') : 0;
        $value = json_encode(['color_temperature' => ['mirek' => $Temperature], 'dynamics' => ['duration' => $duration]]);
        $this->SendDebug(__FUNCTION__, 'Value: ' . $value, 0);
        $result = $this->SendData('PUT', self::RESOURCE_GROUPED_LIGHT, $resourceID, $value);
        return !$this->CheckForErrors($result);
    }
    ...
    /**
     * Sets the scene for the group of lights.
     *
     ...
     * @throws Exception
     */
    private function SetScene(string $SceneID): bool
    {
        $this->SetValue('Scene', $SceneID);
        $value = json_encode(['recall' => ['action' => 'active']]);
        $this->SendDebug(__FUNCTION__, 'Value: ' . $value, 0);
        $result = $this->SendData('PUT', self::RESOURCE_SCENE, $SceneID, $value);
        return !$this->CheckForErrors($result);
    }
