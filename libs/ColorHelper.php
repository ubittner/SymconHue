...
trait ColorHelper
{
    protected function Hex2RGB(int $value): array
    {
        $RGB = [];
        $RGB[0] = (($value >> 16) & 0xFF);
        $RGB[1] = (($value >> 8) & 0xFF);
        $RGB[2] = ($value & 0xFF);
        $this->SendDebug('Hex2RGB', 'R: ' . $RGB[0] . ' G: ' . $RGB[1] . ' B: ' . $RGB[2], 0);
        return $RGB;
    }

    protected function RGB2CIE($red, $green, $blue): array
    {
        ...
        $this->SendDebug('RGB2CIE', 'X: ' . $X . ' Y: ' . $Y . ' Z: ' . $Z, 0);
    ...
    protected function CIE2RGB($x, $y, $brightness = 255): string
    {
        $z = 1.0 - $x - $y;
        $Y = $brightness / 255;
        $X = ($Y / $y) * $x;
